...
This Chapter will discuss improvements made to the PlanetLab application. The improvements are based on the analysis made in the Section~\ref{section:improvement}. The implementation details will be described and shown. The goal of the re-implementation is to make the \texttt{plmbng} tool simple to use, easier to contribute into by re-writing it fully into Python 3, using good coding practices and remove any post-installations steps. First, in Section~\ref{section:implementapproach} the implementation details will be described, then in Section~\ref{section:toolbehavior} the application behavioral diagram will be shown and explained. 
